...
import static io.angularpay.paynable.exceptions.ErrorCode.REQUEST_CANCELLED_ERROR;
...
import static io.angularpay.paynable.models.UserNotificationType.*;
...
public class UpdateRequestStatusCommand extends AbstractCommand<UpdateRequestStatusCommandRequest, GenericCommandResponse>
    ...
    private GenericCommandResponse updateRequestStatus(UpdateRequestStatusCommandRequest request) throws OptimisticLockingFailureException {
        PaynableRequest found = getRequestByReferenceOrThrow(this.mongoAdapter, request.getRequestReference());
        PaynableRequest response = this.commandHelper.updateProperty(found, request::getStatus, found::setStatus);
    ...
    public UserNotificationType getUserNotificationType(GenericCommandResponse commandResponse) {
        RequestStatus status = commandResponse.getPaynableRequest().getStatus();
        switch (status) {
            case ACTIVE:
