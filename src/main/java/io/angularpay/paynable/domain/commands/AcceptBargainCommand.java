...
import static io.angularpay.paynable.models.UserNotificationType.BARGAIN_ACCEPTED;
...
public class AcceptBargainCommand extends AbstractCommand<AcceptBargainCommandRequest, GenericCommandResponse>
    ...
    public String convertToUserNotificationsMessage(UserNotificationBuilderParameters<GenericCommandResponse, PaynableRequest> parameters) throws JsonProcessingException {
        ...
        if (optional.isPresent() && optional.get().equalsIgnoreCase(parameters.getUserReference())) {
            summary = "the bargain you made on a Salary Advance post, was accepted";
        } else {
            summary = "someone's bargain on a Salary Advance post that you commented on, was accepted";
        }
