...
import io.angularpay.paynable.domain.DeletedBy;
...
import static io.angularpay.paynable.domain.DeletedBy.INVESTOR;
import static io.angularpay.paynable.domain.DeletedBy.TTL_SERVICE;
import static io.angularpay.paynable.helpers.CommandHelper.*;
...
import static io.angularpay.paynable.models.UserNotificationType.INVESTOR_DELETED_BY_SELF;
import static io.angularpay.paynable.models.UserNotificationType.INVESTOR_DELETED_BY_TTL;
...
public class RemoveInvestorCommand extends AbstractCommand<RemoveInvestorCommandRequest, GenericCommandResponse>
    ...
    public UserNotificationType getUserNotificationType(GenericCommandResponse commandResponse) {
        DeletedBy deletedBy = commandResponse.getPaynableRequest().getInvestors().stream()
    ...
    public List<String> getAudience(GenericCommandResponse commandResponse) {
        return this.getUserNotificationType(commandResponse) == INVESTOR_DELETED_BY_SELF ?
                getAllPartiesExceptActor(commandResponse.getPaynableRequest(), commandResponse.getItemReference()) :
                getAllParties(commandResponse.getPaynableRequest());
    }
    ...
    public String convertToUserNotificationsMessage(UserNotificationBuilderParameters<GenericCommandResponse, PaynableRequest> parameters) throws JsonProcessingException {
        String summary;
        Optional<String> optional = parameters.getCommandResponse().getPaynableRequest().getInvestors().stream()
        ...
        if (optional.isPresent() && optional.get().equalsIgnoreCase(parameters.getUserReference())) {
            summary = "the comment you made on a Salary Advance post, was deleted because payment wasn't received";
        } else {
            summary = "someone's investment on a Salary Advance post that you commented on, was deleted";
        }
