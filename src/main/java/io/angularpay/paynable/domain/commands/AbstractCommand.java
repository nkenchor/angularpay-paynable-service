...
import io.angularpay.paynable.exceptions.ValidationException;
import io.angularpay.paynable.models.AccessControl;
...
import static io.angularpay.paynable.helpers.Helper.*;
...
public abstract class AbstractCommand<T extends AccessControl, R> {
    ...
    public R execute(T request) {
        try {
            ...
            if (this instanceof UpdatesPublisherCommand && response instanceof PaynableRequestSupplier) {
                log.info("publishing {} update to REDIS => message payload:  {}", this.name, responseText);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((UpdatesPublisherCommand)this).publishUpdates((PaynableRequestSupplier)response);
                });
            }
            if (this instanceof TTLPublisherCommand && response instanceof PaynableRequestSupplier) {
                log.info("publishing {} TTL to REDIS", this.name);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((TTLPublisherCommand)this).publishTTL((PaynableRequestSupplier)response);
                });
            }
            if (this instanceof UserNotificationsPublisherCommand && response instanceof PaynableRequestSupplier) {
                log.info("publishing {} User Notification to REDIS", this.name);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((UserNotificationsPublisherCommand)this).publishUserNotification((PaynableRequestSupplier)response);
                });
            }
