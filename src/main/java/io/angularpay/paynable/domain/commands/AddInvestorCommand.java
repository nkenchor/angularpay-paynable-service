...
import static io.angularpay.paynable.exceptions.ErrorCode.TARGET_AMOUNT_BOUNDS_ERROR;
...
import static io.angularpay.paynable.helpers.Helper.getAllPartiesExceptActor;
import static io.angularpay.paynable.models.UserNotificationType.PEER_INVESTOR_ADDED;
import static io.angularpay.paynable.models.UserNotificationType.SOLO_INVESTOR_ADDED;
...
public class AddInvestorCommand extends AbstractCommand<AddInvestorCommandRequest, GenericReferenceResponse>
        ...
        if (runningTotal.compareTo(targetAmount) < 0 &&
            ...
            Investor investor = Investor.builder()
                    ...
                    .investmentStatus(InvestmentStatus.builder()
                            ...
                            .build())
                    .build();
            PaynableRequest response = this.commandHelper.addItemToCollection(found, investor, found::getInvestors, found::setInvestors);
    ...
    public List<String> getAudience(GenericCommandResponse commandResponse) {
        return getAllPartiesExceptActor(commandResponse.getPaynableRequest(), commandResponse.getItemReference());
    }
    ...
    public String convertToUserNotificationsMessage(UserNotificationBuilderParameters<GenericCommandResponse, PaynableRequest> parameters) throws JsonProcessingException {
        Optional<Investor> optional = parameters.getCommandResponse().getPaynableRequest().getInvestors().stream()
        ...
        String template;
        if (parameters.getCommandResponse().getPaynableRequest().getInvestee().getUserReference()
                .equalsIgnoreCase(parameters.getUserReference())) {
            template = "someone wants to invest %s %s on your Salary Advance post";
        } else {
            template = "someone else wants to invest %s %s on a Salary Advance post that you commented on";
        }
    ...
    @Override
    public String convertToTTLMessage(PaynableRequest paynableRequest, GenericCommandResponse genericCommandResponse) throws JsonProcessingException {
        URI deletionLink = UriComponentsBuilder.fromUriString(configuration.getSelfUrl())
                .path("/paynable/requests/")
