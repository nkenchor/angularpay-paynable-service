...
import static io.angularpay.paynable.models.UserNotificationType.INVESTOR_BARGAIN_ADDED;
...
public class AddBargainCommand extends AbstractCommand<AddBargainCommandRequest, GenericCommandResponse>
    ...
    private GenericCommandResponse addBargain(AddBargainCommandRequest request) throws OptimisticLockingFailureException {
        ...
        Offer offer = Offer.builder()
                .interestRatePercent(request.getAddBargainApiModel().getInterestRatePercent())
        ...
        PaynableRequest response = this.commandHelper.addItemToCollection(found, offer, bargain::getOffers, bargain::setOffers);
    ...
    public String convertToUserNotificationsMessage(UserNotificationBuilderParameters<GenericCommandResponse, PaynableRequest> parameters) throws JsonProcessingException {
        ...
        if (parameters.getCommandResponse().getPaynableRequest().getInvestee().getUserReference()
            ...
            summary = "someone made a bargain on your Salary Advance post";
        } else {
            summary = "someone else made a bargain on a Salary Advance post that you commented on";
        }
